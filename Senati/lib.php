<?php  // $Id: lib.php,v 1.417.2.2 2006/06/01 19:48:37 skodak Exp $
...
//JUSCE
function print_courses_senati($category, $width="100%", $hidesitecourse = false) {
/// Category is 0 (for all courses) or an object
    ...
    if ($courses) {
        foreach ($courses as $course) {
            ...
            print_course_senati($course, $width);
        }
    } else {
        print_heading(get_string("nocoursesyet"));
        if (iscreator()) {  // Make it obvious for newbies on new sites how to add a course
...
//JUSCE
function print_course_senati($course, $width="100%") {
	  $query = 'SELECT visible FROM mdl_course where id='. $course->id  ;
      $result = pg_query($query) or die('Query failed: ' . pg_last_error());
		  
	  while($roxy=pg_fetch_array($result)) 
	         {
				$visa=$roxy["visible"];
		     }	
