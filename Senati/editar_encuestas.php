<?PHP
    ...
    if ($accion=="salvar")
		{
		$numero_registros = count($_POST['inp_id_encuesta']); // Cuantos elementos hay en el array?
		$mensaje="No hay ningún registro a procesar";
		if ($numero_registros > 0)
		    {
			for ($i=0; $i<$numero_registros; $i++)
			     {
				 $id_feedback =$_POST["inp_id_encuesta"][$i];
				 $id_tutor=$_POST["sel_tutor"][$i];
				 
				 if ($id_tutor!="")
				    {$query_update="update mdl_feedback set id_tutor=". $id_tutor . " where id=". $id_feedback ;}
				 else
				    {$query_update="update mdl_feedback set id_tutor=NULL where id=". $id_feedback ;}
	...
	$query = "Select A.*, B.firstname, B.lastname from mdl_feedback A left join mdl_user B on B.id=A.id_tutor where course=" . $id_curso_moodle . " order by A.id asc";
...
// Printing results in HTML
	...
	$titulo_pagina1 = "Edicion de Encuestas a TUTORES";
	print_header("$site->shortname : ". $titulo_pagina1, "X1", $titulo_pagina1, "", "", true, "");

?>
<strong style="color:blue"><a href="view.php?id=<?PHP echo $id_curso_moodle?>"><u><? echo $nombre_moodle ?></u></a> - Edici&oacute;n de Encuestas a TUTORES</strong><BR><BR>
...
<td><?PHP echo $c1 ?></td>
<td align=center><?PHP echo $row["id"] ?><input type=hidden name="inp_id_encuesta[]" value="<?PHP echo $row["id"] ?>"></td>
...
<ul>
<LI><a href="editar_chats.php?id=<?PHP echo $id_curso_moodle ?>">Enlazar Chats</a></LI>
